...
func (c *EventProcessor) StartConsumers(ctx context.Context, kafkaBrokers []string) {
	...
	transactionConsumer := kafkashared.NewConsumer(transactionConfig, c.HandleTransactionEvent)
...
func (c *EventProcessor) HandleTransactionEvent(ctx context.Context, message kafka.Message) error {
	...
	err = c.db.WithTransaction(ctx, func(tx pgx.Tx) error {
		...
		if err := c.transactionService.ProcessTransactionCreated(ctx, tx, event); err != nil {
